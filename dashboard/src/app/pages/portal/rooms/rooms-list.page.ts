...
export class RoomsListPage {
  ...
  readonly rooms = computed(() => this.content()?.rooms ?? []);
  readonly filtered = computed(() => {
    const q = this.query().trim().toLowerCase();
    const items = this.rooms();
    if (!q) return items;
    return items.filter(r => (r.name ?? '').toLowerCase().includes(q) || (r.id ?? '').includes(q));
  });
  ...
  refresh(): void {
    ...
    this.api.getContent().subscribe({
      ...
      error: (err: unknown) => {
        ...
        this.error.set('Falha ao carregar quartos.');
      }
    });
  }
