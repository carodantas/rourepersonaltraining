...
export class BlogService {
  ...
  private readonly content$ = this.http
    .get<CmsContent>(this.apiUrl('content.json'))
    .pipe(shareReplay(1));
  ...
  /**
   * Draft preview (token-based). This bypasses "published" filtering.
   * Token is generated by the dashboard via POST /admin/preview-token.
   */
  getPreviewPostByToken(locale: SupportedLocale, token: string): Observable<BlogPostView | null> {
    const t = (token ?? '').toString().trim();
    if (!t) return of(null);
    return this.http
      .get<{ ok: boolean; post: CmsPost }>(this.apiUrl(`preview/post?token=${encodeURIComponent(t)}`))
      .pipe(
      switchMap((res) =>
        this.content$.pipe(
          map((c) => {
  ...
  private apiUrl(path: string): string {
    ...
    const prefix = (environment.cmsPathPrefix ?? '').trim().replace(/\/$/, '');
