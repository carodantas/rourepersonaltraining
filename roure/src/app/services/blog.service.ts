...
export class BlogService {
   ...
   * Token is generated by the dashboard via POST /api/admin/preview-token.
   */
  getPreviewPostByToken(locale: SupportedLocale, token: string): Observable<BlogPostView | null> {
    const t = (token ?? '').toString().trim();
    if (!t) return of(null);
    return this.http.get<{ ok: boolean; post: CmsPost }>(`api/preview/post?token=${encodeURIComponent(t)}`).pipe(
      switchMap((res) =>
        this.content$.pipe(
          map((c) => {
